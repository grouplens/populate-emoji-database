...
class Emoji:
    ...
    def _get_emoji_article_url(self):
        response = s.get('http://emojipedia.org' + self._url)
    ...
    @property
    def url(self):
        '''The url extension of the emoji
            Example: '/shrug/'
        '''
        # Currently, the only case where there isn't a url set is if Emojipedia.search was used
        # (should already have the url unless use search for random)
        if not self._url:
            raise RuntimeError('No url for emoji {0}'.format(self.title))
        if 'emoji/' in self._url:
            # Resolve to emoji article
            self._url = self._get_emoji_article_url()
        return self._url
